-- 0009: Make claim_invite_atomic idempotent.
--
-- Previously, if a device already had a row in family_devices (from a prior
-- claim) and the admin generated a new invite, the second claim failed with
-- "duplicate key value violates unique constraint family_devices_pkey".
--
-- Fix: check if the device is already a member of this family; if so, skip
-- the inserts and return the existing key material straight away.
-- Also add ON CONFLICT DO NOTHING to both inserts as a belt-and-suspenders
...
BEGIN
  ...
  -- Idempotent path: device already a member of this family — just return keys.
  IF EXISTS (
  ...
  -- Normal path: device not yet in the family.
