...
create policy encrypted_rows_insert on public.encrypted_rows
  for insert with check (
    ...
    and encrypted_rows.written_by_device = uuid_send(auth.uid())
  );
...
create policy key_distribution_select_own on public.key_distribution
  for select using (
    recipient_device_fp = uuid_send(auth.uid())
  );
  ...
  for select using (
    exists (
      ...
      where fd2.family_id = family_devices.family_id
        and fd2.device_fp = uuid_send(auth.uid())
...
-- families: readable by active members of that family.
create policy families_select on public.families
  for select using (
    exists (
      select 1 from public.family_devices fd
      where fd.family_id = families.id
        and fd.device_fp = uuid_send(auth.uid())
        and fd.revoked_at is null
    )
  );
